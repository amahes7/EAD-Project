...
public class StatisticsBean {
	...
	private ArrayList<DescStatistics> descriptiveStatisticsBeanList = new ArrayList<DescStatistics>();
	...
	public String correlateVariables() {
		reset();
		String status = generateCorrReport();
		if (status == "SUCCESS") {
	...
	public void setSelectedTable() {
		...
		columnSelected = dbOperations.getColumnSelected();
		getColumnNames();
	}
	...
	public boolean generateNumericColumnsForScript() {
		try {
	...
	public boolean generateNumericColumns() {
		try {
			numericData.clear();
			categoricalData.clear();
			for (String column : columnSelected) {
				String arr[] = column.split(" ", 2);
				String columnName = arr[0];
				String datatype = arr[1];
				if (datatype.equalsIgnoreCase("char") || datatype.equalsIgnoreCase("varchar")) {
					categoricalData.add(columnName);
				} else
					numericData.add(columnName);
			}
			return true;
		} catch (Exception e) {
			message = e.getMessage();
	...
	public String generateReportForScript() {
		statisticList = new ArrayList<DescStatistics>();
		reset();
		if (tableList.isEmpty()) {
			message = "No tables found in the schema.";
	...
	public String calculateVariables() {
		try {
			for (int k = 0; k < list.size(); k++) {
				String sqlQuery = "select " + list.get(k) + " from " + dbAccess.getSchema() + "." + tableSelected;
				resultSet = dbAccess.processSelect(sqlQuery);
				if (resultSet == null) {
					message = dbAccess.getMessage();
					renderMessage = true;
				...
				for (int j = 1; j < columnCount + 1; j++) {
					...
					statisticList.add(new DescStatistics(columnName, minValue, maxValue, mean, variance, std, median,
							q1, q3, iqr, range));
	...
	public String generateRegressionReport() {
		...
		if (calculateRegressionVariables()) {
			dbAccess.getActionScript().append("\r\n regress\t" + predictorValue + "\t" + responseValue);
			return "SUCCESS";
		} else
			return "FAIL";
	}

	public String generateCorrReport() {
		reset();
		if (tableList.isEmpty()) {
			message = "No tables found in the schema.";
			renderMessage = true;
		...
		if (calculateRegressionVariables()) {
			dbAccess.getActionScript().append("\r\n corr\t" + predictorValue + "\t" + responseValue);
			return "SUCCESS";
		} else
			return "FAIL";
	}
	...
	public boolean calculateRegressionVariables() {
		try {
			...
			String sqlQuery = "select " + predictorValue + ", " + responseValue + " from " + dbAccess.getSchema() + "."
					+ tableSelected;
			resultSet = dbAccess.processSelect(sqlQuery);
			if (resultSet != null) {
				resultSetMetaData = (ResultSetMetaData) resultSet.getMetaData();
				...
				boolean regressionResultsStatus = regression.setRegressionAnalysisVariables(regressionEquation,
	...
	public ArrayList<DescStatistics> getDescriptiveStatisticsBeanList() {
		return descriptiveStatisticsBeanList;
	}

	public void setDescriptiveStatisticsBeanList(ArrayList<DescStatistics> descriptiveStatisticsBeanList) {
		this.descriptiveStatisticsBeanList = descriptiveStatisticsBeanList;
	}
